...
/**
 * Composable for handling voice/video recording with drag-to-lock/cancel gestures.
 * This is framework-agnostic and works with any Vue 3 setup.
 */
export function useChatRecording(
  ...
  const stopRecording = (triggerSend = false) => {
    ...
    if (triggerSend && callbacks.onSend) callbacks.onSend();
