            ...
            <template v-if="localProfile">
                <div class=" pt-16.5 px-2 w-full h-full">
                        ...
                        <div class=" w-full h-29">
                            <BImage
                                ...
                                <div class=" w-full h-full p-2">
                                    <BIcon icon="PhX" class=" fill-white w-5 h-5 cursor-pointer"
                                        @click="closeSidebar" />
                                </div>
                            </BImage>
                        </div>
                    ...
                    <div class=" w-full mt-2 flex flex-col items-center select-none justify-center gap-y-2">
                        <div class=" text-title-md text-on-surface">{{ localProfile.name }}</div>
                        <BLabel color="primary" :text="t('chat.online')" v-if="localProfile.isOnline" />
                    </div>
                    <div class=" w-full px-6">
                        <div class=" w-full py-4 flex items-center justify-center gap-x-2">
                            <div @click="handleAction(action)"
                                ...
                                <BIcon :class="[action.color === 'error' ? 'fill-error' : 'fill-primary']"
                                    class="  w-6 h-6" :icon="action.icon" weight="fill" />
                        ...
                        <div class=" w-full flex flex-col gap-y-4 py-4">
                            <div v-for="(info, index) in displayedInfo" :key="index"
                                ...
                                <div class=" text-body-sm text-on-surface/50">{{ info.title }}</div>
                                <div class=" text-body-md text-on-surface">{{ info.value }}</div>
                            </div>
                        </div>
                    </div>
                </div>
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const localProfile = ref<Contact | null>(null)
