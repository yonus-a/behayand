...
export const useChatStore = defineStore("behayand-chat", () => {
  ...
  const adapter = ref<ChatDataAdapter | null>(null);
  ...
  function setAdapter(newAdapter: ChatDataAdapter) {
  ...
  const fetchConversations = async (
    ...
    state.loading = true;
    try {
      const result = await adapter.value.fetchConversations(
        filter,
        page,
        chatsPerPage.value,
        search,
  ...
  const markAsRead = (conversationId: number) => {
    if (adapter.value) {
      adapter.value.markAsRead(conversationId);
    }
    for (const key in conversationStates.value) {
      const contact = conversationStates.value[key as FilterKeys].data.find(
  ...
  const unreadCount = computed(() => {
    ...
    return Array.from(uniqueContacts.values()).filter(
      (contact) => contact.lastMessage && !contact.lastMessage.isRead,
    ).length;
  });
