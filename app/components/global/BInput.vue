<template>
  <div class="b-input-wrapper">
    <div class="text-label-sm mb-1.5 select-none text-on-surface">
      {{ title }}
    </div>
    <div :style="inputStyle" class="w-full relative">
      ...
      >
        <BMenu @select="handleOptionSelect" :options="options">
          <template #trigger="{ isOpen }">
            <div
              class="flex rtl:pl-4 ltr:pr-4 cursor-pointer text-on-surface/50 items-center gap-x-3"
            >
      ...
      <input
        v-if="!textarea && preset !== 'time'"
        ref="inputField"
        :id="`b-input-${uniqueId}`"
        :key="finalInputType"
        :name="`field-${uniqueId}`"
        :readonly="
          readonly || type === 'date' || (type === 'password' && !isFocus)
        "
        :maxlength="maxlength"
        :type="finalInputType"
        v-model="inputValue"
      ...
      >
        <input
          ref="hoursInput"
          :value="hours"
          @input="(e) => handleTimeInput('h', e)"
          @keydown="(e) => handleTimeKeydown('h', e)"
          @focus="handleTimeFocus('h')"
          @blur="handleTimeBlur('h')"
          :disabled="disabled"
          :readonly="readonly"
        ...
        <input
          ref="minutesInput"
          :value="minutes"
          @input="(e) => handleTimeInput('m', e)"
          @keydown="(e) => handleTimeKeydown('m', e)"
          @focus="handleTimeFocus('m')"
          @blur="handleTimeBlur('m')"
          :disabled="disabled"
          :readonly="readonly || isMinutesLocked"
          ...
          :class="{ 'opacity-30 pointer-events-none': isMinutesLocked }"
          placeholder="00"
          inputmode="numeric"
          maxlength="2"
        />
      ...
      <div
        class="b-input-slots-wrapper max-h-full hide-scrollbar absolute inset-0 pointer-events-none flex justify-between items-center"
        :style="{ paddingInline: inputStyle['--i-pad-internal'] }"
      >
        <div
          ref="startSlotRef"
          class="pointer-events-auto flex items-center shrink-0"
        >
          <span
            v-if="prefix.trim().length > 0"
            class="b-input-prefix select-none"
          >
            {{ prefix }}
          </span>

          <BIcon
            v-else-if="finalIcon.trim().length > 0 && type !== 'phone'"
            :icon="finalIcon"
          ...
          >
            <div class="w-4 h-4 rounded-full overflow-hidden shrink-0">
              <img
                :src="flagUrl"
                class="scale-150 w-full h-full object-cover"
              />
            </div>
        ...
        >
          <span
            v-if="passfix.trim().length > 0"
            class="b-input-passfix select-none"
          >
            {{ passfix }}
          </span>
    ...
    <PasswordQuality
      v-if="newPassword && type === 'password'"
    ...
    <div class="b-input-message-wrapper -translate-y-0 overflow-hidden h-4">
      <div
        ...
        :class="[
          showMessage
            ? `${!newPassword ? 'translate-y-0' : 'translate-y-1'} opacity-100`
...
const props = defineProps({
  ...
  type: {
    type: String as PropType<
      "text" | "password" | "phone" | "number" | "slug" | "date"
    >,
    default: "text",
  },
...
const selectedCountry = computed(() => {
  return (
    defaultCountries.find(
      (c) => c.code.toUpperCase() === selectedCountryCode.value.toUpperCase(),
    ) || defaultCountries
  );
});
...
const finalInputType = computed(() => {
  if (props.type === "password") {
    ...
    if (props.autocomplete === "off" || props.autocomplete === "new-password")
      return "text";
    return "password";
  }
...
const passwordIcon = computed(() =>
  showPassword.value ? "PhEyeSlash" : "PhEye",
);
const finalAutocomplete = computed(() => {
  ...
  if (
    props.type === "password" &&
    (props.autocomplete === "on" || props.autocomplete === "")
  ) {
